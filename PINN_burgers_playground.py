...
class PINN_GAN_burgers(nn.Module):
    ...
    def weight_update(self):
        ...
        for index, w in enumerate([self.domain_weights] + self.boundary_weights): # concatenate lists with domain and boundary weights
            ...
            # TODO make it an option
                    ...
                    #            torch.cat((self.X0, self.u0), 1)))
            if index == 0:
                # print(self.beta(f_update, e) == 1.0)
                

                if torch.sum((self.beta(f_update, e)==1.0).to(torch.int))>0:
                    # print("debug")
    ...
    def train(self, X_star, u_star, epochs = 1e-4, lr_G = 1e-3, lr_D = 5e-3, lr_decay = 0.5, n_critic = 1):
        ...
        but does not help significantly (even causes slower decreasing rate) 
        in the first stage when the loss is still fast decreasing.  
        Improvement might be to activate GAN 
         when the loss stagnates only."""
        # LSTM dynamic training rate: starting 5e-2 lr_decay = 0.7
        optimizer_G = adam.Adam(self.generator.parameters(), lr=lr_G)
        optimizer_D = SGD(self.discriminator.parameters(), lr=lr_D)
